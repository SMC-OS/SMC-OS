...
**Trial reminders — scheduler state found on 23 Sep 2026 (read-only inspection).** `simo-follow-up-production` runs `python -m app.jobs.follow_up` with restart policy `NEVER`. Its only deployment (`99a2351a`, 3 Sep 2026) was built from an uploaded code snapshot — no Git source, no commit recorded — so it does not follow `production-release`. Its logs show daily starts at ~03:00 UTC from 11 to 20 Sep, then none; its `cronSchedule` is now `null`. **The follow-up job appears not to have run since 20 Sep 2026** (see §10's description of the intended schedule). The service also has no `RESEND_API_KEY` or `FRONTEND_BASE_URL`, so it cannot send email.

Before relying on it for trial reminders, the owner must decide and apply, in one approved change: (1) restore the `0 3 * * *` schedule; (2) redeploy it from the same code as the API (for example by connecting it to `SMC-OS/SMC-OS` on `production-release` with the same start command, or a new cron service); (3) add `RESEND_API_KEY`, `FRONTEND_BASE_URL` and `EMAIL_SENDING_DOMAIN` as references to the API service's own variables. What changes for that job: `app/notifications/follow_up_service.py` is unchanged since the 3 Sep snapshot; the only new behaviour is the trial-reminder call. `app/jobs/automations.py` exists in the code but is not run by this service's start command, so it would not start running. Without (3), reminders are a deliberate no-op (they record nothing), never a false "sent". To send reminders by hand meanwhile: `railway ssh --service simo-api-production -- python -m app.jobs.trial_reminders` (idempotent).

**Stripe TEST-mode verification (staging).** `scripts/staging/verify_trial_billing.py` runs inside `simo-api-staging` (stages `setup`, `verify`, `advance`, `cleanup`; refuses anything but an `sk_test_` key and requires `--confirm-staging`). Staging deploys from `main`, so Phase B must be on `main` (or staging's source changed) before this can run.
