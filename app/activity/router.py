...
# gate is require_verified_email, not bare get_current_user.
router = APIRouter(
    prefix="/activity", tags=["activity"], dependencies=[Depends(require_verified_email)]
)
...
def create_activity(event: ActivityEventCreate, current_user: User = Depends(require_verified_email)):
